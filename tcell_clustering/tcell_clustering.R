...
qc_plots <- function(sce_object, prefix, doublets = TRUE, facet_ncol, point_size = 0.3) {
    # Plot clusters
    qc_umap_plots(prefix, sce_object, "label", continuous_scale = FALSE, add_legend = FALSE, file_name = "clusters", text_by = "label", point_size = point_size)
    # Plot samples
...
AGS_sce_tcells_batch1 <- read_rds("../clustering/AGS_sce_filtered_tcells_batch1.rda")
AGS_sce_tcells_batch2 <- read_rds("../clustering/AGS_sce_filtered_tcells_batch2.rda")

AGS_sce_leiden_batch1 <- filter_osca(AGS_sce_tcells_batch1, iterative_filter = TRUE) %>%
    estimate_corral() %>%
    estimate_umap() %>%
    leiden_clustering()
qc_plots(AGS_sce_leiden_batch1, "all_cells_batch1/", doublets = F)
AGS_sce_leiden_batch2 <- filter_osca(AGS_sce_tcells_batch2, iterative_filter = TRUE) %>%
    estimate_corral() %>%
    estimate_umap() %>%
    leiden_clustering()
...
AGS_sce_leiden_batch1_good <- dplyr::filter(AGS_sce_leiden_batch1, !is_in(label, c(2,10))) %>%
    filter_osca(filter_cells = FALSE) %>%
    estimate_corral() %>%
    estimate_umap() %>%
    leiden_clustering()
...
AGS_sce_leiden_batch1_good2 <- dplyr::filter(AGS_sce_leiden_batch1_good, !is_in(label, c(6))) %>%
    filter_osca(filter_cells = FALSE) %>%
    estimate_corral() %>%
    estimate_umap() %>%
    leiden_clustering()
qc_plots(AGS_sce_leiden_batch1_good2, "all_cells_batch1_good2/", doublets = F)
catch <- map(marker_genes, umap_plot, "all_cells_batch1_good2/", AGS_sce_leiden_batch1_good2, height = 6, width = 7)

AGS_sce_leiden_batch1_good3 <- dplyr::filter(AGS_sce_leiden_batch1_good2, !is_in(label, c(3,5))) %>%
    filter_osca(filter_cells = FALSE) %>%
    estimate_corral() %>%
    estimate_umap() %>%
    leiden_clustering()
qc_plots(AGS_sce_leiden_batch1_good3, "all_cells_batch1_good3/", doublets = F)
catch <- map(marker_genes, umap_plot, "all_cells_batch1_good3/", AGS_sce_leiden_batch1_good3, height = 6, width = 7)

AGS_sce_list <- list("1" = AGS_sce_leiden_batch1_good3, "2" = AGS_sce_leiden_batch2)
AGS_sce_tcells_merge <- merge_batches(AGS_sce_list)
AGS_sce_leiden <- estimate_umap(AGS_sce_tcells_merge, "corrected") %>%
    leiden_clustering("corrected")
qc_plots(AGS_sce_leiden, "all_cells/", doublets = F)

AGS_sce_leiden_good <- dplyr::filter(AGS_sce_leiden, !is_in(label, c(5,6,10))) %>%
    estimate_umap("corrected") %>%
    leiden_clustering("corrected")
qc_plots(AGS_sce_leiden_good, "all_cells_good/", doublets = F)

# T-cell marker mess - to clean up later
naive_or_TCM <- c("CCR7","SELL","TCF7")
T_act1 <-  c("IL2","TNF","IL4R")
T_act2 <-  c("IFNG","CCL4","CCL3")
Trm <-  c("CXCR6","ITGA1")
Treg <- c("FOXP3","IL2RA","CTLA4")
Temra <- c("PRF1","NKG7")
#T_cytokines <-  c("CCL3","CCL4","CCL20","IFNG","IL10","TNF", "LAG3","TIGIT","HAVCR2","CTLA4","PDCD1","2B4","CD200")
T_inhibitory <- c("LAG3","TIGIT","HAVCR2")
T_effector_mem_diff <-  c("ZEB2","EOMES","ZNF683")
T_ifnresponse <-  c("IFIT3","IFIT2","STAT1","MX1","IRF7","JAK2")
T_prolif <- c("MKI67", "NME1","IFIT3")
T_mait <- c("KLRB1", "NCR3", "ZBTB16", "SLC4A10", "RORC")
T_NK_Azi_heart_ct1 <- c("PARP8", "IL7R", "THEMIS", "AOAH", "SKAP1", "CD247", "ITK", "PTPRC", "CAMK4", "GNLY", "CD3E")
T_Azi_heart_ct2 <- c("PARP8", "IL7R", "THEMIS", "AOAH", "SKAP1", "CD247", "ITK", "PTPRC", "CAMK4", "GNLY", "CD3E")
T_mait_Azi <-  c("KLRB1", "NKG7", "GZMK", "SLC4A10", "NCR3", "CTSW", "IL7R", "KLRG1", "CEBPD", "DUSP2")
T_gd1_Azi <-  c("TRDC", "TRGC1", "TRGV9", "TRDV2", "KLRD1", "IL7R", "KLRC1", "DUSP2", "GNLY", "KLRG1")
T_stemlike <-  c("TCF7")
T_naive_like <-   c("IL7R")
T_costimulatory <- c("CD28", "TNFRSF14", "ICOS", "TNFRSF9")
T_transcription_factors <- c("EOMES", "HOPX",  "TBX21", "ZEB2", "ZNF683", "HIF1A", "ID2", "TOX", "ZBTB16")
T_cyt_effector <- c("FGFBP2","KLRG1","CXCR1")
T_dysfunct <- c("CXCL13")
T_cytokines <-  c("IFNG","TNF")
T_prolif2 <-  c("LIF", "IL2", "CENPV", "NME1", "FABP5", "ORC6", "G0S2", "GCK") 
NK_adaptive_Huuhtanen <-  c("FCGR3A", "KLRC2","ZEB2")
NK_adaptive <-  c("KLRC2","FCGR3A","NCAM1","SH2D1B","CD3E","TRDC") ##Besides KLRC2, ALL TEHSE ARE ALSO EXPRESSED BY CYTOTXIC Tgd cell, exercise caution
nk_cd56_bright        <- c("NCAM1", "KLRC1", "MYC", "BACH2")
nk_cd56_dim           <- c("FCGR3A", "PRDM1", "ZEB2", "MAF")
nk_adaptive           <- c("KLRC2", "KLF2")
nk_mark               <- list(nk_cd56_bright = nk_cd56_bright, nk_cd56_dim = nk_cd56_dim, nk_adaptive = nk_adaptive)

#catch <- map(marker_genes, UMAP_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(naive_or_TCM, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_act1, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_act2, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(Trm, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(Treg, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(Temra, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_cytokines, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7) 
catch <- map(T_inhibitory, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_effector_mem_diff, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_ifnresponse, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_prolif, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_mait, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_NK_Azi_heart_ct1, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_Azi_heart_ct2, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_mait_Azi, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_gd1_Azi, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_prolif, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_stemlike, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_naive_like, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_inhibitory, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_costimulatory, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_transcription_factors, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_cyt_effector, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_dysfunct, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(T_prolif2, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(NK_adaptive_Huuhtanen, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(NK_adaptive, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(nk_cd56_bright, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(nk_cd56_dim, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)
catch <- map(nk_adaptive, umap_plot, "all_cells/", AGS_sce_leiden, height = 6, width = 7)

umap_plot("TCL1A", "all_cells/", AGS_sce_leiden)
umap_plot("CD68", "all_cells/", AGS_sce_leiden)
umap_plot("XCL1", "all_cells/", AGS_sce_leiden)
catch <- map(c("GZMA", "GZMB", "GZMH", "GZMK"), umap_plot, "all_cells/", AGS_sce_leiden)
catch <- map(c("CD4", "CD8A", "CD8B"), umap_plot, "all_cells/", AGS_sce_leiden)
trbv_genes <- rownames(AGS_sce_leiden) %>% str_subset("TRBV") %>% str_subset("OR9", negate = TRUE)
trc_genes <- map(c("TRBC1", "TRBC2", "TRAC"), umap_plot, "all_cells/", AGS_sce_leiden)
catch <- map(trbv_genes, umap_plot, "all_cells/", AGS_sce_leiden)

umap_plot("CD79A", "all_cells/", AGS_sce_leiden)
umap_plot("CD19", "all_cells/", AGS_sce_leiden)
umap_plot("CD14", "all_cells/", AGS_sce_leiden)
umap_plot("CST3", "all_cells/", AGS_sce_leiden)
umap_plot("CD44", "all_cells/", AGS_sce_leiden)
umap_plot("SIK3", "all_cells/", AGS_sce_leiden)
umap_plot("FYN", "all_cells/", AGS_sce_leiden)

umap_plot("REL", "all_cells/", AGS_sce_leiden)
umap_plot("CREM", "all_cells/", AGS_sce_leiden)
umap_plot("PTPRC", "all_cells/", AGS_sce_leiden)
umap_plot("ZNF280C", "all_cells/", AGS_sce_leiden)
umap_plot("RPS27", "all_cells/", AGS_sce_leiden)
umap_plot("RPS12", "all_cells/", AGS_sce_leiden)
umap_plot("FOS", "all_cells/", AGS_sce_leiden)

# Differential expression between clusters
AGS_markers <- findMarkers(AGS_sce_leiden, test = "t", direction = "up", pval.type = "some", assay.type = "logcounts", row.data = rowData(AGS_sce_leiden))
AGS_markers_df <- as.list(AGS_markers) %>% map(as_tibble) %>% bind_rows(.id = "Cluster") %>% select(Cluster:GENESEQEND, hvg, p.value:logFC.1)
write_tsv(AGS_markers_df, "t_cell_markers.tsv")
...
celltype_labels <- c(
    ...
    "3" = "Cytotoxic T-cells",
    "4" = "Cytotoxic T-cells",
    "5" = "Effector memory T-cells",
    "6" = "Effector memory T-cells",
    "7" = "Effector memory T-cells",
    "8" = "TCL1A+ T-cells",
    "9" = "Active NK cells",
    "10" = "Unknown",
    "11" = "Effector memory T-cells",
    "12" = "Effector memory T-cells",
    "13" = "Effector memory T-cells",
    "14" = "Effector memory T-cells",
    "15" = "Macrophages",
    "16" = "T-regs"
)

AGS_sce_celltypes <- add_celltype_labels(AGS_sce_leiden, celltype_labels, new_celltypes = F)
